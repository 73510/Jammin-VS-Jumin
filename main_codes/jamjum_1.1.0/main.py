...
class init_object:
    ...
    def juminsharkinit():
        ...
        shark.vel = [jumin.vel[0] + throwconst, jumin.vel[1]]
    ...
    def jamminsharkinit():
        ...
        shark.vel = [jammin.vel[0] - throwconst, jammin.vel[1]]


    def jamminrst():
        jammin.lock = True
        jammin.show = True
        jammin.pos[0] = width // 10 * 8
...
def get_key(events):
    ...
    for event in events:  #나중에 키보드 입력 함수 따로 만들기
        ...
        if event.type == pygame.KEYDOWN:
            ...
            elif event.key == pygame.K_LEFTBRACKET: keytense_arrow[0] = 1
            elif event.key == pygame.K_QUOTE : keytense_arrow[1] = 1
            elif event.key == pygame.K_SEMICOLON: keytense_arrow[2] = 1
            elif event.key == pygame.K_RETURN: keytense_arrow[3] = 1
            ...
            if not (jamshoot_var['loving'] or jamshoot_var['throwing']) : #재민이가 날아가는중, 사랑하는 중엔 공격 불가
                if event.key == pygame.K_p : pygame.event.post(shark_var[2])

            if event.key == pygame.K_RSHIFT and  jamshoot_var["throwing"]  == False : #충전키가 눌렸고 날아가는 중이 아닐때
                shiftpressed = True
                jamshoot_var['thrown_tick'] = pygame.time.get_ticks()
            elif event.key == pygame.K_SLASH and jamshoot_var["throwing"]  == False and  jamshoot_var["charged"]: #발사키가 눌렸고, 충전이 되었고 날아가는 중이 아니면
            ...
            elif event.key == pygame.K_LEFTBRACKET: keytense_arrow[0] = 0
            elif event.key == pygame.K_QUOTE : keytense_arrow[1] = 0
            elif event.key == pygame.K_SEMICOLON: keytense_arrow[2] = 0
            elif event.key == pygame.K_RETURN: keytense_arrow[3] = 0
...
def i_got_shot():
    ...
    if jammin.collision(badminton) and badminton_var[3] == True:
        oof.play()
        punch.play()
    ...
    if jumin.collision(badminton) and badminton_var[4] == True:
    ...
    if jammin.collision(shark) and shark_var[3] == True:
        oof.play()
        punch.play()
    ...
    if jumin.collision(shark) and shark_var[4] == True:
    ...
    if pygame.time.get_ticks() - love_start_t >= 3000 and jamshoot_var['loving'] == True:
        oof.play()
        damage = jamshoot_var['thrown_tick']//80
        JUMIN_HP -= damage
        init_object.jamminrst()
        love.show = False
        lovesounds.stop()
...
def ultimate_screensiba():
    ...
    for i in range (70):
        ...
        pygame.time.wait(16)
        pygame.display.flip()
    jumin.pos = [0 + width // 10 * 2, height // 2]
...
def main_play():
    ...
    while running:
        ...
        if pygame.time.get_ticks()-sibastarttick >= 10000 and sibaactivate == True:
            ...
            jammin.pos = [width // 10 * 8, height // 2]
            jumin.pos = [0 + width // 10 * 2, height // 2]
            jammin.lock = True
            jumin.lock = True
            shark_var[4] = False
        ...
        if sibaactivate == False :
            ...
            if jamshoot_var["throwing"] :
                jammin.lock = False
                jammin.vel = [-jamshoot_var["thrown_tick"]/20,0]
